...
impl ManagedCursor {
    ...
    pub fn value(&self) -> Result<Option<JsValue>, Error> {
        self.inner.as_ref().map(|cursor| cursor.value()).transpose()
    }
...
impl From<Cursor> for ManagedCursor {
    fn from(inner: Cursor) -> Self {
